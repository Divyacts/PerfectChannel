...
public class TC42 extends TestBase {
	...
	public void executeTest() {
		try {
			System.out.println("Test to perform Credit batching Ad-Hoc");
			SignOnPage signOnPage = new SignOnPage(remoteDriver);
			signOnPage.enterUrl();
			JdeHomePage jdeHomePage = signOnPage.loginAsValidUser();
			SOPEnquiryPage sopEnquir = jdeHomePage.navigateToSOP();
			String flagforAdHoc = "creditAdHocBatching";
			sopEnquir.verifyREcordsPresentForSale(flagforAdHoc);
			...
			SaleBatchSubmissionPage saleBatch = jdeHomePage 
					.navigateToCreditBatchingAdHoc();
			saleBatch.submitCombineBtachingForAdHoc();
			jdeHomePage.goToHomeScreen();
			Thread.sleep(1000);
